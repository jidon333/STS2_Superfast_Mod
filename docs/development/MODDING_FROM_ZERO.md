...
- `baseSpeed = 3.0`, `spineSpeed = 1.0` -> 애니메이션 3배속
- `baseSpeed = 3.0`, `queueSpeed = 1.0` -> 대기시간은 3배속 기준으로 더 짧아짐
- `baseSpeed = 3.0`, `effectSpeed = 1.0` -> 타이머 지연도 3배속 기준으로 더 짧아짐
...
1. `STS2_SPEED_*`
2. `Sts2Speed.config.json`
3. `Sts2Speed.speed.txt`
4. 기본값

현재 권장 방식은 `Sts2Speed.config.json`이다. `Sts2Speed.speed.txt`는 기존 사용자 설정이 바로 깨지지 않도록 남겨 둔 fallback이다.
...
반대로 현재 `effectSpeed`가 겨냥한 `CombatState.GodotTimerTask(double)`는 utility timer helper라서, 설정은 살아 있어도 체감에 항상 크게 보이지는 않을 수 있다.
...
- `PENDING_HOOKS_AND_RISKS.md`
  - 왜 일부 훅은 아직 안 붙였는지, 왜 `effectSpeed`가 덜 눈에 띄는지 설명
