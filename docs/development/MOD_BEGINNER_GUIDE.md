...
현재 이 저장소는 완성된 “배속 모드 제품”이라기보다 다음 네 가지를 묶은 작업 저장소다.
...
즉 “모드가 안전하게 로드되고, 되돌릴 수 있고, 패치를 추가할 수 있는 구조”를 만드는 것이 1차 목적이었다.
...
- 모드 저장 경로가 `modded/profile1`로 분리된다
