...
GUMM은 초기에 로더 진입을 검증하기 위한 역사적 실험 경로였고, 현재 저장소의 실제 툴링/배포 경로에는 포함하지 않는다.
...
이 후보 훅은 "아직 구현을 못 해서"라기보다, 디컴파일 결과 전투 동기화/액션 큐 핵심 루프에 가까워서 일부러 보류한 것이다.

자세한 이유는 `PENDING_HOOKS_AND_RISKS.md`를 보면 된다.
