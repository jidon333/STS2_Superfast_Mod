...
처음에는 `prefs.save`, `settings.save`, `progress.save`, `current_run.save`를 기준으로 백업 설계를 세웠다.
...
- reflection만으로 밀기보다는 디컴파일 도구를 쓰는 게 빠르다
...
- `Sts2Speed.speed.txt = 1.25` 를 두면 런타임 설정 로그에 `enabled=True spine=1.25 queue=1.25 effect=1.25`가 남는다
