...
## 29. 커뮤니티 배포 예시가 방향을 바꿈

이후 추가로 확인한 커뮤니티 배포 예시는 설치법이 매우 단순했습니다.

- `Slay the Spire 2\mods` 폴더 생성
- 그 안에 `pck`, `dll`, `txt` 파일 투입
- `txt` 파일의 숫자를 수정해 배속 변경

이 정보가 중요한 이유:

- GUMM이라면 보통 `override.cfg`, `GUMM_mod_loader.tscn` 같은 선행 설치가 필요합니다.
- 그런데 그 설명에는 그런 단계가 없었습니다.

즉, 이 배포 형태는 GUMM보다 게임 내장 `mods` 경로를 더 강하게 시사합니다.

## 30. 로컬 설치본 문자열 재확인

커뮤니티 배포 예시를 본 뒤, 설치본 문자열 단서를 다시 확인했습니다.
...
이건 적어도 다음 정도는 강하게 시사합니다.

- 게임 내부에 `mods` 디렉토리 개념이 있다.
- `.pck` 기반 로딩 경로가 있다.
- managed initializer 계약이 있다.

따라서 이 시점부터는 GUMM보다 네이티브 `mods + pck + dll + txt`를 최종 방향으로 보는 것이 더 자연스러워졌습니다.

## 31. 네이티브 스테이징 도구 추가

그래서 저장소에는 실제 네이티브 스테이징 명령을 추가했습니다.

새 명령:

- `materialize-native-package --layout subdir`
- `materialize-native-package --layout flat`

이 명령이 하는 일:

- `mods` 폴더 기준 산출물 생성
- DLL 복사
- `Sts2Speed.speed.txt` 생성
- 네이티브 로더 힌트 JSON 생성
- 현재 빠져 있는 `.pck`를 누락 필수 아티팩트로 명시

즉, 아직 live deploy는 막아두되, 어떤 파일이 어디에 놓여야 하는지는 툴에서 고정되도록 만들었습니다.

## 32. 현재 결론의 수정본

지금 기준 결론은 이전보다 더 선명합니다.

- GUMM은 실제로 되는 경로다.
- 하지만 GUMM은 최종 목표가 아니라 부트스트랩/진단 경로다.
- 최종 구현 목표는 게임 내장 `mods + pck + dll + txt` 경로다.
- 현재 남은 진짜 blocker는 `.pck` 생성/패키징 규약 확정이다.
